...
class TcpConnectionTable(collections.abc.Mapping):
    ...
    def _determine_path_for_pid(self, pid):
        p = psutil.Process(pid)
        try:
            return p.exe()
        except psutil.AccessDenied:
            return str(pid) # pid 0, the system idle process, is a pseudo-process on windows and doesn't have a path

    def get_path_for_pid(self, pid):
        return self._pid_table[pid]
    ...
    def _refresh_ipv4(self):
        ...
        if ret == 0:
            for row in self._tcp.table[:self._tcp.dwNumEntries]:
                ...
                self._pid_table[row.dwOwningPid] = self._determine_path_for_pid(row.dwOwningPid)
...
class RedirectLocal(Redirect):
    ...
    def handle(self, packet):
        ...
        pid_path = self.tcp_connections.get_path_for_pid(pid)

        if pid not in self.trusted_pids and pid_path not in self.trusted_processes:
