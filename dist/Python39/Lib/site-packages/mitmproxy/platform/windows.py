...
def find_port(port_file: str, default_port: int) -> int:
    port_path = os.path.join(os.environ['LOCALAPPDATA'], 'SkyWall', port_file)
...
REDIRECT_API_PORT = find_port('redirectport.txt', 8085)
...
class RedirectLocal(Redirect):
    ...
    whitelisted_paths: typing.Set[str]
    exact_whitelisted_paths: typing.Set[str]
    blacklisted_paths: typing.Set[str]
    exact_blacklisted_paths: typing.Set[str]
    ...
    def __init__(
        ...
        # logging.basicConfig(filename="C:\\Users\\Public\\Documents\\skywall-logs\\windows.log", format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', level=logging.DEBUG)
        self.tcp_connections = TcpConnectionTable()
        self.trusted_pids = set()
        self.whitelisted_paths = set()
        self.exact_whitelisted_paths = set()
        self.pid_paths = {}
        self.blacklisted_paths = set()
        self.exact_blacklisted_paths = set()
        ...
        processes_dir = os.path.join(script_file_path, "processes")
        hosts_file = os.path.join(processes_dir, "processes.json")
        ...
        observer = Observer()
        observer.schedule(handler, processes_dir, recursive=False)
        observer.start()
    ...
    def load_file(self, src_path):
        with self.lock:
            self.exact_whitelisted_paths.clear()
            self.exact_blacklisted_paths.clear()
            with open(src_path) as f:
                data = json.load(f)
                self.whitelisted_paths.clear()
                self.whitelisted_paths.update(data.get('whitelistedPaths', set()))
                self.blacklisted_paths.clear()
                self.blacklisted_paths.update(data.get('blacklistedPaths', set()))

    def determine_pid_path(self, pid):
        if pid is None:
            return ''
        handle = None
        try:
            handle = self.kernel32.OpenProcess(0x1000, 0, pid)
            if handle < 1:
                # logging.info("Got invalid handle for pid " + str(pid))
                return ''
            else:
                # char buffer of double the normal windows path limit, should be safe
                path = (ctypes.c_wchar * 520)()
                size = ctypes.c_uint(520)
                status = self.kernel32.QueryFullProcessImageNameW(handle, 0, path, ctypes.byref(size))
                if status < 1:
                    # logging.info("Got invalid status from query for pid " + str(pid))
                    return ''  # maybe raise exception
                else:
                    return path.value
        except Exception as e:
            # logging.error("Got unknown exception for pid " + str(pid))
            # logging.error(e)
            return ''
        finally:
    ...
    def is_windows_pid(self, pid, pid_path):
        if pid is None:
            return False
        elif (pid_path.startswith("C:\\Windows") or pid < 100) and pid_path.find("powershell.exe") == -1 and pid_path.find("cmd.exe") == -1:
            # logging.info("Pid " + str(pid) + " is a windows pid")
            return True
        else:
            # logging.info("Pid " + str(pid) + " is not a windows pid")
            return False

    def is_process_in_whitelisted_dir(self, pid_path: str) -> bool:
        if pid_path in self.exact_whitelisted_paths:
            return True

        for path in self.whitelisted_paths:
            if pid_path.startswith(path):
                self.exact_whitelisted_paths.add(pid_path)
                return True
        return False

    def is_process_in_blacklisted_dir(self, pid_path: str) -> bool:
        if pid_path in self.exact_blacklisted_paths:
            return True

        for path in self.blacklisted_paths:
            if pid_path.startswith(path):
                self.exact_blacklisted_paths.add(pid_path)
                return True
        return False
    ...
    def handle(self, packet):
        ...
        if pid_possibly_reused or pid not in self.pid_paths:
            # start = time.perf_counter_ns()
            pid_path = self.determine_pid_path(pid)
            # logging.info("Took " + str((time.perf_counter_ns() - start)) + "ns to lookup process path")
            # logging.info("Obtained pid path of " + pid_path + " for pid " + str(pid))
            self.pid_paths[pid] = pid_path
        else:
            pid_path = self.pid_paths.get(pid, None)

        if pid in self.trusted_pids or self.is_process_in_whitelisted_dir(pid_path) or self.is_windows_pid(pid, pid_path):
        ...
        elif not self.is_process_in_blacklisted_dir(pid_path):
...
class TransparentProxy:
    ...
    def __init__(
        ...
        self.proxy_port = find_port('port.txt', proxy_port)
        self.filter = (
                filter
                or
                f"tcp.DstPort != {self.proxy_port} and tcp.DstPort != {REDIRECT_API_PORT} and tcp.DstPort < 49152"
        )
        ...
        self.response = Redirect(
            ...
            f"outbound and tcp.SrcPort == {self.proxy_port}",
        )
