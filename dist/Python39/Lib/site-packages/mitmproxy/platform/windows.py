...
class RedirectLocal(Redirect):
    ...
    trusted_processes: typing.Set[str]
    known_pid_paths: typing.Set[str]
    ...
    def loadFile(self, srcPath):
        with self.lock:
            self.known_pid_paths.clear()
            with open(srcPath) as f:
                data = json.load(f)
                self.trusted_processes.clear()
                self.trusted_processes.update(data.get('processes', set()))
    ...
    def is_process_in_whitelisted_dir(self, pid_path):
        if pid_path in self.known_pid_paths:
            return True

        for path in self.trusted_processes:
            if pid_path.startswith(path):
                self.known_pid_paths.add(pid_path)
                return True
        return False
    ...
    def handle(self, packet):
        ...
        if pid not in self.trusted_pids and not self.is_process_in_whitelisted_dir(pid_path) and not self.is_windows_pid(pid, pid_path):
