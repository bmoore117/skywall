...
public class UpdateServiceJob extends Job implements ActivatableJob {
    private static final Logger log = LoggerFactory.getLogger(UpdateServiceJob.class);

    public UpdateServiceJob() {}

    public UpdateServiceJob(LocalDateTime jobLaunchTime, String jobDescription, Service service, ActivationStatus newStatus) {
    ...
    public Boolean call(ApplicationContext applicationContext) {
        ...
        configService.withTransaction(config -> configService.withFilterTransaction(filterConfig -> {
            ConfigUtils.findServiceById(config, getActivatableId()).ifPresent(service -> service.updateCurrentActivationStatus(getNewStatus()));
            ...
            List<String> pendingIgnoredHosts = config.getDefinedServices().stream()
                    .filter(service -> service.getCurrentStatus() == ActivationStatus.PENDING_ACTIVATION
                            || service.getCurrentStatus() == ActivationStatus.NEEDS_REACTIVATION)
                    .map(service -> {
                        HashSet<String> remainingHosts = service.getHosts().stream()
                                .filter(host -> PASSTHROUGH == host.getFilterMode())
            ...
            List<String> pendingHosts = config.getDefinedServices().stream()
                    .filter(service -> service.getCurrentStatus() == ActivationStatus.PENDING_ACTIVATION
                            || service.getCurrentStatus() == ActivationStatus.NEEDS_REACTIVATION)
                    .map(service -> {
                        HashSet<String> remainingHosts = service.getHosts().stream()
                                .filter(host -> STANDARD == host.getFilterMode())
                                .map(Host::getHost).collect(Collectors.toCollection(HashSet::new));
            ...
            for (Service service : configService.getConfig().getDefinedServices()) {
                if (service.getCurrentStatus() != ActivationStatus.DISABLED) {
