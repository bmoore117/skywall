...
public class DeleteBlacklistedPathJob extends Job implements ActivatableJob {

    public DeleteBlacklistedPathJob() {}

    public DeleteBlacklistedPathJob(LocalDateTime jobLaunchTime, String jobDescription, Path path) {
    ...
    public Boolean call(ApplicationContext applicationContext) {
        ...
        configService.withTransaction(config -> config.setBlacklistedPaths(config.getBlacklistedPaths().stream()
                .filter(path -> !activatableId.equals(path.getId())).collect(Collectors.toList())));
