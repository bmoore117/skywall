...
import com.hyperion.jarvis.backend.model.config.Credentials;
...
public class ConfigService {
    ...
    public boolean importDefinedServices(InputStream inputStream) {
        try {
            com.hyperion.jarvis.backend.model.config.service.Service[] newServices =
                    mapper.readValue(inputStream, com.hyperion.jarvis.backend.model.config.service.Service[].class);

            Map<String, com.hyperion.jarvis.backend.model.config.service.Service> currentServices =
                    config.getDefinedServices().stream().collect(Collectors.toMap(
                            com.hyperion.jarvis.backend.model.config.service.Service::getName, Function.identity()));
            for (com.hyperion.jarvis.backend.model.config.service.Service newService : newServices) {
                if (!currentServices.containsKey(newService.getName())) {
                    currentServices.put(newService.getName(), newService);
                } else {
                    com.hyperion.jarvis.backend.model.config.service.Service current = currentServices.get(newService.getName());
